...
aliases:
  - "features/tracking/bitemporal history"
---

Every `LexModel` automatically tracks changes along two independent time dimensions — *valid time* (when something was true in the real world) and *system time* (when the system recorded the change). This builds on the [[history-and-audit/history|standard history layer]] (which records *what* changed) by adding a second table — **MetaHistory** — that records *when the system learned about each change*.

> [!tip]
> If you haven't read it yet, start with [[history-and-audit/history|History (Simple History)]] — it explains the foundation that this page extends.
