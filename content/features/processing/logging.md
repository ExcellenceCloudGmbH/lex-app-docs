...
- **PDF Export** — the calculation log for any record can be exported as a PDF,
  useful for compliance documentation and auditing. The PDF uses the same
  Markdown features as the log view, so tables, fenced code blocks, and even
  strikethrough stay readable in the exported file.
