...
| `SESSION_SECRET`                          | Fixed secret used to sign dashboard session cookies. Required for HTTPS-style deployments unless you explicitly allow ephemeral dev sessions. `SESSION_KEY` and `SESSION_SECRET_KEY` are accepted aliases. |
| `LEX_ALLOW_EPHEMERAL_SESSION_SECRET`      | Set `true` only for single-process development if you want to run without `SESSION_SECRET`. Users will be logged out after restarts. |
