...
> [!important]
> **One file per model class.** Input models go in `Input/`, upload models in `Upload/`, report models in `Reports/`. This follows the [[start-here/project structure|ETL convention]].
...
By inheriting from `LexModel`, your model automatically gets a database table, REST API endpoints, [AG Grid](https://www.ag-grid.com/)-powered frontend, `created_by`/`edited_by` tracking, and full [[history-and-audit/bitemporal history|bitemporal history]]. We recommend starting every data model from `LexModel` — see [[reference/LexModel Internals]] for a complete list of what it provides.
...
By default, Lex App generates a basic API serializer for every model. We recommend adding custom validation for any model where data quality matters. Create a `serializers.py` file alongside the model it applies to — this uses [Django REST Framework](https://www.django-rest-framework.org/) serializers. See [[model-your-data/serializers]] for the full guide.
...
> [!tip]
> The `@add_permission_checks` decorator ensures your serializer respects the [[access-and-dashboards/permissions|permission system]]. Validation errors show directly in the frontend UI.
...
> [!tip]
> For the full reference — including nested sub-groups, model styling, and the `untracked_models` knob — see [[model-your-data/model structure]].
...
> [!tip]
> This section covers just enough to get your tutorial data loaded. For the full reference — including `update` and `delete` actions, file uploads, and advanced patterns — see [[model-your-data/initial data|Initial Data Upload]].
...
Next up: [[start-here/tutorial/Part 3 — Calculations & Logging|Part 3 — Calculations & Logging]] where you'll build `BudgetSummary` in the `Reports/` folder.
